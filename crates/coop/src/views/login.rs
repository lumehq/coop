...
use anyhow::anyhow;
use app_state::constants::BUNKER_TIMEOUT;
...
impl Login {
    ...
    fn login_with_bunker(&mut self, content: &str, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            this.update_in(cx, |this, window, cx| {
                match result {
                    Ok(uri) => {
                        this.save_connection(&app_keys, &uri, cx);
    ...
    fn save_connection(&mut self, keys: &Keys, uri: &NostrConnectURI, cx: &mut Context<Self>) {
        ...
        if let Some(secret) = uri.secret() {
            clean_uri = clean_uri.replace(secret, "");
        }

        // Write the app keys to the key store
        key_store
            .read(cx)
            .store(KeyItem::Bunker, secret, cx)
            .detach();

        // Write the connect uri to the key store
        key_store
            .read(cx)
            .store(KeyItem::User, clean_uri, cx)
            .detach();
    }
    ...
    pub fn login_with_keys(&mut self, keys: Keys, cx: &mut Context<Self>) {
        ...
        // Write the current key to the key store
        key_store.read(cx).store(KeyItem::User, secret, cx).detach();
